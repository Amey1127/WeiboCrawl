...
class WeiboSearchCrawler(SinaSearchCrawler):
    ...
    def extract_publish_time(self, weibo, tag):      
        if(type(tag) != types.NoneType):
            ...
            if(type(link) != types.NoneType):
                ...
                # 微博ID
                    weibo.weiboID = weibo.weibourl.split('/')[-1].split('?')[0]
    ...
    Output:
        weibo:已经赋值的微博信息
    """
    def extract_forward_comment_like_count(self, weibo, tag):
        if(type(tag) != types.NoneType):
            ...
            if (type(like) != types.NoneType):
                ...
                if (m):
                    weibo.likes_count = int(m.group(0))
    """
        从网页中抽取微博用户信息
    Input:
        html: 搜索结果网页
            ...
            if(type(header_json) == types.DictType):
                ...
                for wb in wb_list:
                    ...
                    if(len(p_time) > 0 ):
                        ...
                        print "extract publish time error:%s %s" % (weibo.writerID, weibo.screen_name)
                        continue
                        
                    
                    #转发收藏评论数
                    wb_action = wb.find("div", class_="feed_action clearfix")
                    self.extract_forward_comment_like_count(weibo, wb_action)
